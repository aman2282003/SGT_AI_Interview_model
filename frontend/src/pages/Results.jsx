      ...
      <div className="bg-white dark:bg-gray-900 rounded-[2rem] shadow-xl border border-gray-100 dark:border-gray-800 overflow-hidden relative transition-colors duration-300">
        ...
        <div className="p-8 md:p-14">
          <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-10 pb-10 border-b border-gray-100 dark:border-gray-800 gap-8">
            ...
            <div className={`flex flex-col items-center justify-center p-8 rounded-3xl border-2 min-w-[14rem] shadow-sm transform hover:scale-105 transition-all duration-300 ${isEvaluating ? 'bg-indigo-50 dark:bg-indigo-900/20 border-indigo-200 dark:border-indigo-900/30' : session.aiMarks >= 80 ? 'bg-green-50 dark:bg-green-900/20 border-green-200 dark:border-green-900/30' : session.aiMarks >= 50 ? 'bg-amber-50 dark:bg-amber-900/20 border-amber-200 dark:border-amber-900/30' : 'bg-red-50 dark:bg-red-900/20 border-red-200 dark:border-red-900/30'}`}>
              <span className="text-sm font-bold uppercase tracking-widest text-gray-600 dark:text-gray-400 mb-2 transition-colors duration-300">{isEvaluating ? 'AI Status' : 'Total AI Score'}</span>
              <div className="flex items-baseline">
                {isEvaluating ? (
                  <div className="flex flex-col items-center">
                    <div className="animate-spin rounded-full h-8 w-8 border-b-2 border-indigo-600 mb-2"></div>
                    <span className="text-xl font-bold text-indigo-600 dark:text-indigo-400">Evaluating...</span>
                  </div>
                ) : (
                  <>
                    <span className={`text-7xl font-black tracking-tighter transition-colors duration-300 ${isEvaluating ? 'text-indigo-600 dark:text-indigo-400' : session.aiMarks >= 80 ? 'text-green-600 dark:text-green-400' : session.aiMarks >= 50 ? 'text-amber-500 dark:text-amber-400' : 'text-red-500 dark:text-red-400'}`}>{session.aiMarks}</span>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-12">
            <div>
              ...
              <div className="prose prose-indigo prose-lg text-gray-700 dark:text-gray-300 bg-purple-50 dark:bg-purple-900/10 p-8 rounded-3xl border border-purple-100 dark:border-purple-900/30 leading-relaxed font-medium shadow-inner transition-colors duration-300">
